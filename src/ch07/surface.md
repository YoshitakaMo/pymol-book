...
`surface_circumscribe`は、PyMOLの内部で使用されるパラメータであり、通常は変更する必要はありません。デフォルトは`2.0`です。

### surface_clear_cutoff

(float, default: 0.0) controls the cutoff distance from surface_clear_selection atoms which surface points must meet to be excluded for the carved surface.

`surface_clear_selection`で選択された原子からの距離がこの値よりも大きい場合、その点は彫り取った表面から除外されません。デフォルトは`0.0`です。

### surface_clear_selection

carveされた表面から除外する原子のnamed selectionを指定します。`surface_clear_cutoff`と組み合わせて使用されます。string型で、デフォルトは空白です。

### surface_clear_state

クリアする原子の状態を指定します。デフォルトは`0`で、対応する座標セットの状態が使用されます。
...
surfaceを描画する際に、どの原子を考慮するかを制御します。デフォルトは`0`で、ignoreフラグとexfoliateフラグに基づいて原子が考慮されます。`1`に設定するとすべての原子が考慮され、`2`に設定すると水素以外の原子のみが考慮されます。

|`surface_mode=0`|`surface_mode=1`|
|---|---|
|![surface_mode0](./image/surface/smode0.png)|![surface_mode1](./image/surface/smode1.png)|
...
surfaceの表示を行う時に用いる三角形の数を制御します。デフォルトは`0`です。`2`以上は小さな分子に対してのみ実用的で、`2`未満は大きな分子に対してのみ実用的です。タンパク質などの系では描画コストが高くなりすぎるため、`0`または`1`に設定するのがよいでしょう。

|`surface_quality=0`|`surface_quality=1`|`surface_quality=2`|
|---|---|---|
|![surface_quality0](./image/surface/squality0.png)|![surface_quality1](./image/surface/squality1.png)|![surface_quality2](./image/surface/squality2.png)|
