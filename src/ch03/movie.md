...
### Append
この機能はおもに下記のProgram機能を使って先に動きをプログラミングさせておいた後で使います。そして、その動きの動作時間を延長させるためのメニューです。

メニュー上では`0.25`〜`60` secondの延長が可能です。このとき、設定されている[FPS](#FPS)の値（後述）に応じて延長されるFrame数が増加します。つまり、30FPSに設定されているときに2 second延長しようとすると、60 Frame分追加されることになります。
